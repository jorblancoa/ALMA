...
Remember to do `alma-cli getblockcount` to check if VPS catching blocks till it synced with chain, if not follow this procedure:

* Go to your wallet-qt and check peers list (tools - peers list) and select one ip from the list. With that ip do the follow command at VPS `alma-cli addnode "ip" onetry`

      Example:
		  alma-cli addnode 45.32.144.158 onetry
    
* Check now if VPS already downloading blocks with the command `alma-cli getblockcount`, and if yes give it time now to catch last block number 
...
```
alma-cli getmasternodestatus
```

You need to get **"status" : 4** 

## Congratulations your Alma node it's running
